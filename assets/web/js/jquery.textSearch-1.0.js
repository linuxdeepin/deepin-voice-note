...
(function($){
	$.fn.textSearch = function(str,isdiff,options){
		...
		$(this).each(function(){
			...
			if('&nbsp;'.indexOf(str)>=0){
				v_html = v_html.replace(/&nbsp;/g,"jkjkjkkjkjj%a");
			}
			if('&gt;'.indexOf(str)>=0){
				v_html = v_html.replace(/&gt;/g,"hkhkhkhkhkhk%m");
			}
			if('&lt;'.indexOf(str)>=0){
				v_html = v_html.replace(/&lt;/g,"mnmnnmnmnmnm%k");
			}
			if('&amp;'.indexOf(str)>=0){
				v_html = v_html.replace(/&amp;/g,"&");
			}
			...
			if(!isdiff && /[a-zA-Z]/.test(str)){
				...
				var new_html = a.join("");
				new_html=new_html.replace(/hkhkhkhkhkhk%m/g,'&gt;');
				new_html = new_html.replace(/mnmnnmnmnmnm%k/g,"&lt;");
				new_html = new_html.replace(/jkjkjkkjkjj%a/g,"&nbsp;");
				$(this).html(new_html);
			}else{
				$.each(a, function(i, c){
					if(!/<(?:.|\s)*?>/.test(c)){//非标签
