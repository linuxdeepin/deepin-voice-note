...
async function insertImg(urlStr) {
    ...
    urlStr.forEach((item) => {
        ...
        // 增加路径验证
        if (!relPath.startsWith('images/') || relPath.includes('..')) {
            console.error('Invalid image path:', relPath);
            return;
        }

        // 验证文件扩展名
        if (!/\.(jpg|jpeg|png|bmp)$/i.test(relPath)) {
            console.error('Invalid image extension:', relPath);
            return;
        }
