# 语音记事本|../common/deepin_voice_note.svg|
...
- 在语音记事本界面点击 ![icon_menu](../common/icon_menu.svg)，选择 **退出** 来退出语音记事本。
...
- 当第一次新建记事本后，系统默认帮您创建一个文本，此时在文本列表内单击 ![add](../common/circlebutton_add2.svg) 按钮可继续创建新文本。
...
2. 单击 ![finish](../common/finish_normal.svg) 按钮完成录音。
   ...
   > ![notes](../common/notes.svg)说明：录音的限制时长为60分钟。
...
3. 在搜索文本框中点击 ![0|three_dots](../common/close_icon.svg) 清除当前输入的关键字或取消搜索。
...
> ![tips](../common/tips.svg)窍门：您也可以右键单击录音文件，选择 **保存为MP3** 将录音文件保存。


### 添加文字笔记

1. 在语音记事本界面上单击 ![mic](../common/circlebutton_add2.svg) 按钮，添加新的文本笔记。
...
>![tips](../common/tips.svg)窍门：录制音频时可同时开启语音听写的功能。
...
>![notes](../common/notes.svg)说明：当只选中录音文件时，文本翻译不可操作；若选中的录音文件包含语音转文字内容，可进行文本翻译的操作。
...
记事文件完成后，文件将自动保存名为“记事本（n）”，其中n≥0，n为整数，您可以重命名记事本。
