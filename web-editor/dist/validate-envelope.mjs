...
class StepResult {
  ...
  */
  static fromReplace(doc, from2, to, slice2) {
    try {
      return StepResult.ok(doc.replace(from2, to, slice2));
    } catch (e) {
...
function canSplit(doc, pos, depth = 1, typesAfter) {
  let $pos = doc.resolve(pos), base2 = $pos.depth - depth;
...
function replaceStep(doc, from2, to = from2, slice2 = Slice.empty) {
  if (from2 == to && !slice2.size)
    return null;
  let $from = doc.resolve(from2), $to = doc.resolve(to);
...
function getMarksBetween(from2, to, doc) {
  const marks = [];
  if (from2 === to) {
    doc.resolve(from2).marks().forEach((mark) => {
      const $pos = doc.resolve(from2);
...
var setNodeSelection = (position) => ({ tr, dispatch }) => {
  if (dispatch) {
    const { doc } = tr;
    const from2 = minMax(position, 0, doc.content.size);
    const selection = NodeSelection.create(doc, from2);
    tr.setSelection(selection);
  }
  return true;
};
...
.voicePlayback .voiceBtn {
    ...
    background-image: url("data:image/svg+xml,%3Csvg width='24' height='24' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Ccircle cx='12' cy='12' r='12' fill='white'/%3E%3Cpath d='M10.9504 7.29356C9.87321 6.58879 9 7.1975 9 8.64331V15.3569C9 16.8072 9.87675 17.4091 10.9504 16.7067L16.1935 13.2762C17.2706 12.5715 17.2671 11.4265 16.1935 10.724L10.9504 7.29356Z' fill='%230058DE'/%3E%3C/svg%3E");
}

.voicePlayback .voiceBtn:hover {
    background-image: url("data:image/svg+xml,%3Csvg width='24' height='24' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Ccircle cx='12' cy='12' r='12' fill='white'/%3E%3Cpath d='M10.9504 7.29356C9.87321 6.58879 9 7.1975 9 8.64331V15.3569C9 16.8072 9.87675 17.4091 10.9504 16.7067L16.1935 13.2762C17.2706 12.5715 17.2671 11.4265 16.1935 10.724L10.9504 7.29356Z' fill='%230058DE'/%3E%3Ccircle cx='12' cy='12' r='12' fill='white' fill-opacity='.2'/%3E%3C/svg%3E");
}

.voicePlayback .voiceBtn:active {
    background-image: url("data:image/svg+xml,%3Csvg width='24' height='24' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Ccircle cx='12' cy='12' r='12' fill='white'/%3E%3Cpath d='M10.9504 7.29356C9.87321 6.58879 9 7.1975 9 8.64331V15.3569C9 16.8072 9.87675 17.4091 10.9504 16.7067L16.1935 13.2762C17.2706 12.5715 17.2671 11.4265 16.1935 10.724L10.9504 7.29356Z' fill='%230058DE'/%3E%3Ccircle cx='12' cy='12' r='12' fill='black' fill-opacity='.2'/%3E%3C/svg%3E");
}
