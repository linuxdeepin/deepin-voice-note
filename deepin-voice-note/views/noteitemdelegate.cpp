...
struct VNoteNamePHelper {
    VNoteNamePHelper(QPainter *painter, QFontMetrics fontMetrics, QRect nameRect)
...
void VNoteNamePHelper::spiltByKeyword(const QString &text, const QRegExp &keyword)
{
...
void VNoteNamePHelper::paintFolderName(bool isSelected)
{
...
NoteItemDelegate::NoteItemDelegate(NoteListView *parent)
    : DStyledItemDelegate(parent)
    , m_parentView(parent)
{
    m_parentPb = DApplicationHelper::instance()->palette(m_parentView);
    connect(DApplicationHelper::instance(), &DApplicationHelper::themeTypeChanged, this,
            &NoteItemDelegate::handleChangeTheme);
}

void NoteItemDelegate::setSearchKey(const QRegExp &key)
{
    m_searchKey = key;
}

void NoteItemDelegate::handleChangeTheme()
{
    m_parentPb = DApplicationHelper::instance()->palette(m_parentView);
...
void NoteItemDelegate::setEditorData(QWidget *editor, const QModelIndex &index) const
{
    ...
    VNoteItem *data = m_parentView->getItemData(index);
    if(data){
                                    ...
                                    const QModelIndex &index) const
{
    ...
    m_parentView->itemEditFinsh(index,text);
}

void NoteItemDelegate::updateEditorGeometry(QWidget *editor, const QStyleOptionViewItem &option,
                                            const QModelIndex &index) const
{
    Q_UNUSED(index)
    QLineEdit *edit = static_cast<QLineEdit *>(editor);
...
void NoteItemDelegate::paintItemBase(QPainter *painter, const QStyleOptionViewItem &option,
...
void NoteItemDelegate::paintNormalItem(QPainter *painter, const QStyleOptionViewItem &option,
                                       const QModelIndex &index) const
{
    VNoteItem *data = m_parentView->getItemData(index);
    if(!data){
...
void NoteItemDelegate::paintSearchItem(QPainter *painter, const QStyleOptionViewItem &option,
                                       const QModelIndex &index) const
{
    VNoteItem *noteData = m_parentView->getItemData(index);
    if(!noteData){
        return;
    }
    QRect paintRect(option.rect.x() + 10, option.rect.y() + 5,
    ...
    VNoteNamePHelper vfnphelper(painter, fontMetrics, nameRect);
    vfnphelper.spiltByKeyword(noteData->noteTitle, m_searchKey);
    vfnphelper.paintFolderName(isSelect);
    ...
    VNoteFolder *folderData = m_parentView->getFolderData(noteData->folderId);
    if (folderData) {
